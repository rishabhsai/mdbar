...
class NumberMarkerWidget extends WidgetType {
  ...
  constructor(indentSpaces: number, marker: string) {
    super();
    this.indentSpaces = indentSpaces;
    this.marker = marker;
  }
  ...
  override eq(other: NumberMarkerWidget) {
    return (
      other.indentSpaces === this.indentSpaces && other.marker === this.marker
    );
  }
  ...
  for (const visibleRange of view.visibleRanges) {
    ...
    while (position <= visibleRange.to) {
      ...
      if (headingMatch) {
        ...
        const markerEnd =
          markerStart + headingMatch[2].length + headingMatch[3].length;
...
function buildMarkdownPresentationDecorations(
  view: EditorView,
): DecorationSet {
